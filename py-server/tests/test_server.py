...
GEOJSON_DATA_FILE = os.path.join(os.path.dirname(__file__), 'data/other/markers.json')
...
class ServerTest(unittest.TestCase):
  ...
  def test_get_shp_prj_dbf_files_from_tree_has_shp_dbf_prj(self):
    ...
    files_to_return = get_shp_prj_dbf_files_from_tree(current_dir + '/data/test')
  ...
  def test_get_data_from_files_no_files(self, mock_tempfile, mock_shutil, mock_extract_zip):
  ...
  @mock.patch('server.shutil')
  @mock.patch('server.tempfile')
  def test_get_data_from_files(self, mock_tempfile, mock_shutil, mock_extract_zip, mock_get_srid, mock_get_encoding):
    mock_tempfile.mkdtemp.return_value = '/data/test'
    mock_shutil.rmtree.return_value = True
    files_to_return = {
      'shp': 'data/test/streetshighways892015_749_210833.shp',
    ...
    self.assertEqual(data['shape'], 'data/test/streetshighways892015_749_210833.shp')
  ...
  @mock.patch('server.request')
  def test_import_shapefile_shp2pgsql_no_data(self, mock_request, mock_get_data):
  ...
  @mock.patch('server.get_data_from_files')
  @mock.patch('server.request')
  def test_import_shapefile_shp2pgsql_no_shape(self, mock_request, mock_get_data):
    mock_request.method = 'POST'
  ...
  @mock.patch('server.get_data_from_files')
  @mock.patch('server.request')
  def test_import_shapefile_shp2pgsql(self, mock_request, mock_get_data, mock_get_geojson):
    mock_request.method = 'POST'
    mock_get_data.return_value = {
      'shape': 'test'
    }
